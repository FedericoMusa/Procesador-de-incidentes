...
Cada test valida los campos críticos usando el texto real de los PDFs provistos.
"""
...
class TestYPFExtractor:
    ...
    def test_num_inc(self, data):
        assert data['NUM_INC'] == 'YPF-0000246524'

    def test_area_concesionada(self, data):
        assert 'CHIHUIDO' in data['AREA_CONCE']

    def test_yacimiento(self, data):
        assert 'DESFILADERO' in data['YACIMIENTO']
    ...
    def test_coordenadas_negativas(self, data):
        # Sur y Oeste siempre negativos
        assert data['Y_COORD'] < 0
        assert data['X_COORD'] < 0

    def test_coordenadas_valores(self, data):
        assert abs(data['Y_COORD'] - (-37.348933)) < 0.0001
        assert abs(data['X_COORD'] - (-69.053400)) < 0.0001
...
class TestPetSudExtractor:
    ...
    def test_num_inc(self, data):
        assert data['NUM_INC'] == 'PETSUD-562'

    def test_area(self, data):
        assert 'Ventana' in data['AREA_CONCE']
...
class TestPluspetrolExtractor:
    ...
    def test_num_inc(self, data):
        assert data['NUM_INC'] == 'PP-06/26'

    def test_area(self, data):
        assert data['AREA_CONCE'] == 'JCP'

    def test_yacimiento(self, data):
        assert data['YACIMIENTO'] == 'JCP'
    ...
    def test_coordenadas_dd_directas(self, data):
        assert abs(data['Y_COORD'] - (-37.4246588)) < 0.0001
        assert abs(data['X_COORD'] - (-68.4049142)) < 0.0001

    def test_coords_gk_presentes(self, data):
        # Pluspetrol también reporta Gauss-Krüger
        assert data['GK_X_M'] == 5858159.0
        assert data['GK_Y_M'] == 2552673.0
    ...
    def test_codigo_presente(self, data):
        assert data['CODIGO'] == 'DC_DR_0008_26'
...
class TestAconcaguaExtractor:
    ...
    def test_num_inc_usa_subtipo(self, data):
        # Sin número de comunicado propio, usa subtipo de instalación
        assert data['NUM_INC'] == 'ACO-CH-28'

    def test_area(self, data):
        assert 'Herrados' in data['AREA_CONCE']
    ...
    def test_coordenadas_dd_con_signo(self, data):
        # Ya vienen negativas en el PDF
        assert data['Y_COORD'] == -33.3465
        assert data['X_COORD'] == -68.9873
    ...
    def test_responsable(self, data):
        assert 'Singarella' in data['RESPONSABLE']
...
class TestPCRExtractor:
    ...
    def test_num_inc(self, data):
        assert 'MDZ-21' in data['NUM_INC']

    def test_area(self, data):
        assert 'Sosneado' in data['AREA_CONCE']
    ...
    def test_lat_aproximada(self, data):
        # 34°57'51.5" S ≈ -34.964°
        assert abs(data['Y_COORD'] - (-34.964)) < 0.01

    def test_lon_aproximada(self, data):
        # 69°31'59.52" O ≈ -69.533°
        assert abs(data['X_COORD'] - (-69.533)) < 0.01
    ...
    def test_responsable(self, data):
        assert 'Estegui' in data['RESPONSABLE']
